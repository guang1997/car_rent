...
<div style="display: none;padding: 20px" id="saveOrUpdateDiv">
    <form class="layui-form" lay-filter="dataFrm" id="dataFrm">
        <div class="layui-form-item">
            ...
            <%--                <input type="hidden" dtree-id="selTree1" dtree-node="nodeId" name="selTree1_select_nodeId" value=""--%>
            <%--                       readonly="">--%>
            <%--                <input type="text" dtree-id="selTree1" id="selTree1_select_input_id" name="selTree1_select_input"--%>
            <%--                       placeholder="请选择" value="" readonly="" class="layui-input layui-unselect">--%>
...
<script type="text/javascript">
    ...
    layui.extend({
        ...
        tableIns = table.render({
            ...
            , url: '${PATH}/menu/loadAllMenu' //数据接口
        ...
        $("#doSearch").click(function () {
            ...
            tableIns.reload({
                url: "${PATH}/menu/loadAllMenu?" + params
            })
        });
        ...
        table.on('tool(menuTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                    ...
                    //向服务端发送删除指令
                    $.post("${PATH}/menu/checkMenuHasChildren", {id:data.id}, function (res1) {
                        if (res1.code >= 0) {
                            layer.msg("该节点有子节点，请先删除子节点")
                        } else {
                            layer.confirm('真的删除'+data.title+'这个菜单么', function (index) {
                                $.post("${PATH}/menu/delMenuById", {id:data.id}, function (res2) {
                                    layer.msg(res2.msg);
                                    //刷新数据 表格
                                    tableIns.reload();
                                    //刷新左边的树
        ...
        <%--    elem: "#menuTree",--%>
        <%--    width: "100%", // 指定树的宽度--%>
        <%--    url: "${PATH}/menu/loadMenuManagerLeftTreeJson?spread=1"--%>
        <%--});--%>
        var menuTree = dtree.render({
            ...
            url: "${PATH}/menu/loadMenuManagerLeftTreeJson?spread=1",  // 使用url加载（可与data加载同时存在）
            icon: "2",
            accordion: true
        });
    ...
    function reloadTable(id) {
        // alert(id);
        tableIns.reload({
            url: "${PATH}/menu/loadAllMenuById?id=" + id
        })
    }
</script>
</body>
